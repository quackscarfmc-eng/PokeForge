...
[${s.constantName}] =
{
    ...
    .genderRatio = ${s.genderRatio >= 255 ? "MON_GENDERLESS" : `PERCENT_FEMALE(${Math.round(s.genderRatio * 100 / 254)})`},
...
// ---------------------------------------------------------------------------
// Wild encounters code generator (mirrors src/data/wild_encounters.json)
// ---------------------------------------------------------------------------
...
export function generateEncountersCode(encounters: EncounterData[]): string {
  if (encounters.length === 0) return "// No wild encounters defined yet.";
  const byMap = new Map<string, EncounterData[]>();
  ...
  const lines: string[] = [
    "// ============================================================",
    "// Wild encounters (paste into src/data/wild_encounters.json)",
    "// Generated by PokeForge",
    "// ============================================================",
    "{",
  ];
  let firstMap = true;
  for (const [mapLabel, encs] of byMap) {
    if (!firstMap) lines.push(",");
    firstMap = false;
    lines.push(`  "${mapLabel}": {`);
    lines.push(`    "label": "${encs[0].location}",`);
    const byMethod = new Map<string, EncounterData[]>();
    for (const e of encs) {
      if (!byMethod.has(e.method)) byMethod.set(e.method, []);
      byMethod.get(e.method)!.push(e);
    }
    let firstMethod = true;
    for (const [method, mEncs] of byMethod) {
      if (!firstMethod) lines.push(",");
      firstMethod = false;
      lines.push(`    "${method}_mn": {`);
      lines.push(`      "encounter_rate": ${mEncs[0].encounterRate},`);
      lines.push(`      "mons": [`);
      mEncs.forEach((e, i) => {
        const comma = i < mEncs.length - 1 ? "," : "";
        const held = e.heldItemConstant ? `,\n            "held_item": "${e.heldItemConstant}"` : "";
        const form = e.formId != null ? `,\n            "form": ${e.formId}` : "";
        lines.push(`        {`);
        lines.push(`          "min_level": ${e.minLevel},`);
        lines.push(`          "max_level": ${e.maxLevel},`);
        lines.push(`          "species": "${e.speciesConstant}"${held}${form}`);
        lines.push(`        }${comma}`);
      });
      lines.push(`      ]`);
      lines.push(`    }`);
...
export function validateEncounter(e: EncounterData, knownSpecies: string[]): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  if (!e.mapLabel?.startsWith("MAP_")) issues.push({ severity: "warning", message: "Map label should start with MAP_" });
  if (!e.speciesConstant?.startsWith("SPECIES_")) issues.push({ severity: "error", message: "speciesConstant must start with SPECIES_" });
  if (!knownSpecies.includes(e.speciesConstant) && !e.speciesConstant.startsWith("SPECIES_"))
    issues.push({ severity: "warning", message: `Species ${e.speciesConstant} may not exist` });
  if (e.minLevel < 1 || e.minLevel > 100) issues.push({ severity: "error", message: "minLevel must be 1–100" });
  if (e.maxLevel < 1 || e.maxLevel > 100) issues.push({ severity: "error", message: "maxLevel must be 1–100" });
  if (e.minLevel > e.maxLevel) issues.push({ severity: "error", message: "minLevel cannot exceed maxLevel" });
  if (e.encounterRate < 0 || e.encounterRate > 100) issues.push({ severity: "error", message: "encounterRate must be 0–100" });
  const validMethods = ["grass", "water", "rock_smash", "fishing_old", "fishing_good", "fishing_super"];
  if (!validMethods.includes(e.method)) issues.push({ severity: "error", message: `Unknown method: ${e.method}` });
  return issues;
}

// ---------------------------------------------------------------------------
// Trainer code generator (mirrors src/data/trainers.json)
// ---------------------------------------------------------------------------
export interface TrainerPartyMemberData {
  speciesConstant: string;
  level: number;
  iv: number;
...
export interface TrainerData {
  ...
  items: string[];
  party: TrainerPartyMemberData[];
}

export function generateTrainerCode(t: TrainerData): string {
  const partyStr = t.party.length
    ? t.party.map((p, i) => {
        const moves = (p.moves.length ? p.moves : ["MOVE_NONE", "MOVE_NONE", "MOVE_NONE", "MOVE_NONE"])
          .slice(0, 4)
          .map((m) => `              "${m}"`)
          .join(",\n");
        return `          {
            "iv": ${p.iv},
            "level": ${p.level},${p.abilityConstant ? `\n            "ability": "${p.abilityConstant}",` : ""}${p.heldItemConstant ? `\n            "held_item": "${p.heldItemConstant}",` : ""}
            "species": "${p.speciesConstant}",
            "nature": "${p.natureConstant || "NATURE_HARDY"}",${p.gender ? `\n            "gender": "${p.gender}",` : ""}${p.isShiny ? `\n            "is_shiny": true,` : ""}${p.ballConstant ? `\n            "ball": "${p.ballConstant}",` : ""}${p.formId != null ? `\n            "form": ${p.formId},` : ""}
            "moves": [
${moves}
            ]
          }${i < t.party.length - 1 ? "," : ""}`;
  ...
  const itemsStr = t.items.length
    ? t.items.map((i) => `      "${i}"`).join(",\n")
    : "      // no items";

  const aiFlagsStr = t.aiFlags.length
    ? t.aiFlags.map((f) => `      "${f}"`).join(",\n")
    : "      \"AI_FLAG_CHECK_BAD_MOVE\"";

  const intro = (t.introText || "").replace(/"/g, '\\"');
  const defeat = (t.defeatText || "").replace(/"/g, '\\"');
  const rematch = (t.rematchDefeatText || "").replace(/"/g, '\\"');

  return `// ============================================================
// Trainer: ${t.trainerName} (${t.trainerClass})
// Generated by PokeForge — paste into src/data/trainers.json
// ============================================================
{
  "trainer_class": "${t.trainerClass}",
  "name": "${t.trainerName}",
  "party_size": ${t.partySize || t.party.length},
  "party_type": "${t.doubleBattle ? "Double" : "Single"}",
  "ai_flags": [
${aiFlagsStr}
  ],
  "items": [
${itemsStr}
  ...
  "intro": "${intro}",
  "defeat": "${defeat}",
  "rematch_defeat": "${rematch}",
  "rematch_num": ${t.rematchNum},
  "party": [
${partyStr}
  ]
}`;
...
export function validateTrainer(t: TrainerData, knownSpecies: string[], knownMoves: string[]): ValidationIssue[] {
  const issues: ValidationIssue[] = [];
  if (!t.trainerName) issues.push({ severity: "error", message: "Trainer name is required" });
  if (!t.trainerClass?.startsWith("TRAINER_CLASS_")) issues.push({ severity: "warning", message: "Trainer class should start with TRAINER_CLASS_" });
  if (t.party.length === 0) issues.push({ severity: "warning", message: "Party is empty" });
  if (t.party.length > 6) issues.push({ severity: "error", message: "Party cannot exceed 6 Pokémon" });
  for (const [i, p] of t.party.entries()) {
    if (!p.speciesConstant?.startsWith("SPECIES_"))
      issues.push({ severity: "error", message: `Party member ${i + 1}: species must start with SPECIES_` });
    else if (!knownSpecies.includes(p.speciesConstant))
      issues.push({ severity: "warning", message: `Party member ${i + 1}: ${p.speciesConstant} may not exist` });
    if (p.level < 1 || p.level > 100)
      issues.push({ severity: "error", message: `Party member ${i + 1}: level must be 1–100` });
    if (p.iv < 0 || p.iv > 31)
      issues.push({ severity: "error", message: `Party member ${i + 1}: IV must be 0–31` });
    if (p.moves.length > 4)
      issues.push({ severity: "error", message: `Party member ${i + 1}: max 4 moves` });
  }
  return issues;
}
